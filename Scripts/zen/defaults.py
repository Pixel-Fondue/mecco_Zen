...
def get_environments():
    """Returns the names of all environment items in the enforced zen shader tree structure."""
    return ENVIRONMENTS



def get_environment(key):
    """Returns the name of a single environment item in the enforced zen shader tree structure by key."""
    return [i for i in ENVIRONMENTS if i[0] == key]



def get_mask_groups():
    """Returns the names of all parent groups in the enforced zen shader tree structure."""
    return list_of_tuples_dump(MASK_GROUPS)



def get_mask_group(key):
    """Returns the name of a single parent group in the enforced zen shader tree structure by key."""
