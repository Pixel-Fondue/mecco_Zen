...
class zen_isolate(lxu.command.BasicCommand):
    ...
    def CMD_EXE(self, msg, flags):
        ...
        if lx.eval("query scriptsysservice userValue.isDefined ? zen_isolate")==0:
            lx.eval( 'user.defNew zen_isolate boolean' );
            lx.eval( 'user.def zen_isolate username value:0' );

        zen_isolate = lx.eval('user.value zen_isolate ?')

        if zen_isolate==0:
            try:
                lx.eval('hide.unsel')
                lx.eval('item.refSystem')
                lx.eval('user.value zen_isolate 1')
            except Exception:
                lx.out(traceback.format_exc())
                lx.eval('layout.createOrClose EventLog "Event Log_layout" title:@macros.layouts@EventLog@ width:600 height:600 persistent:true open:true')
            ...
            try:
                lx.eval('unhide')
                lx.eval('item.refSystem {}')
                lx.eval('user.value zen_isolate 0')
            except Exception:
                lx.eval('unhide')
                lx.out(traceback.format_exc())
                lx.eval('layout.createOrClose EventLog "Event Log_layout" title:@macros.layouts@EventLog@ width:600 height:600 persistent:true open:true')
    ...
    def checkState(self):
		if lx.eval("query scriptsysservice userValue.isDefined ? zen_isolate")==0:
			lx.eval( 'user.defNew zen_isolate boolean' );
			lx.eval( 'user.def zen_isolate username value:0' );
		return int(lx.eval('user.value zen_isolate ?'))
